...
import CAE.furnace.utils as utils
import torch
...
class VisionTransformer_FT(nn.Module):
...
class VisionTransformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=512, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=None, use_rel_pos_bias=False, 
                 use_shared_rel_pos_bias=False, use_mean_pooling=True, init_scale=0.001, lin_probe=False, 
                 linear_type='standard', args=None):
        super().__init__()
            ...
            if self.linear_type == 'standard':
                ...
                self.attentive_blocks = nn.ModuleList([
                    AttentiveBlock(
                        ...
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer) 
                        for i in range(1)])
                self.norm = utils.LP_BatchNorm(embed_dim, affine=False)
        else:
            if use_mean_pooling:
    ...
    def _checkpoint_block(self, blk, x, rel_pos_bias):
        def custom_forward(*inputs):
            x = inputs[0]
            return blk(x, rel_pos_bias=rel_pos_bias)
    ...
    def forward_features(self, x, is_train=True):
        ...
        if self.use_checkpoint:
            for blk in self.blocks:
                x = self._checkpoint_block(blk, x, rel_pos_bias)
        else:
            for blk in self.blocks:
                x = blk(x, rel_pos_bias=rel_pos_bias)
        ...
        if self.lin_probe:
            if self.linear_type == 'standard':
                ...
                for blk in self.attentive_blocks:
                    query_tokens = blk(query_tokens, x, 0, 0, bool_masked_pos=None, rel_pos_bias=None)
                return self.norm(query_tokens[:, 0, :], is_train=is_train) 
        else:   # finetune
            if self.norm is not None:    # use mean pooling
                t = x[:, 1:, :]
...
class VisionTransformer_LP(nn.Module):
    """ The differences between VisionTransformer_LP and VisionTransformer is :
       1) Adds linear probing support with `lin_probe` parameter and two modes: 'standard' and 'attentive'
       2) Uses conditional normalization: LP_BatchNorm for attentive mode, no norm for standard mode  
       3) Introduces learnable query_token and additional attentive_blocks for attentive probing
       5) Supports is_train parameter for training/inference mode switching in BatchNorm

       Further improve linear probing performance
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=512, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=None, use_rel_pos_bias=False, 
