...
from CAE.models.modeling_finetune import _cfg
from timm.models.registry import register_model
from timm.models.layers import trunc_normal_ as __call_trunc_normal_
from CAE.models.modeling_cae_helper import *
...
class ContextAutoencoderViT(nn.Module):
    def __init__(
        ...
        patch_size=16,                 # Patch size for dividing the image
        in_chans=3,                    # Number of input channels (3 for RGB images)
        embed_dim=768,                 # Embedding dimension (hidden size of the encoder)
        depth=12,                      # Number of encoder layers
        num_heads=12,                  # Number of attention heads in the encoder
        mlp_ratio=4.0,                 # MLP expansion ratio
        qkv_bias=True,                 # Include bias in QKV projections
        qk_scale=None,                 # QK scale factor (None means default)
        drop_rate=0.0,                 # Dropout rate
        attn_drop_rate=0.0,            # Attention dropout rate
        drop_path_rate=0.1,            # Stochastic depth rate (from --drop_path 0.1)
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  # Normalization layer
        init_values=0.1,               # Layer scale initial values (from --layer_scale_init_value 0.1)
        attn_head_dim=None,            # Attention head dimension (None means default)
        init_std=0.02,                 # Standard deviation for weight initialization
        decoder_embed_dim=768,         # Decoder embedding dimension (same as encoder)
        regressor_depth=4,             # Regressor depth (from --regressor_depth 4)
        decoder_num_classes=8192,      # Number of classes for decoder output (from --model cae_base_patch16_224_8k_vocab)
        decoder_num_heads=12,          # Number of decoder attention heads
        decoder_layer_scale_init_value=0.1,  # Decoder layer scale initial values (from --decoder_layer_scale_init_value 0.1)
        decoder_depth=4,               # Decoder depth (from --decoder_depth 4)
        fix_init_weight=False,         # Whether to fix initial weights
        **kwargs                       # Additional keyword arguments
    ):
        super().__init__()
        self.model_type = 'caev2'
        ...
        if self.model_type == 'caev2':
            self.num_out_dim = 512 # kwargs['args']['num_out_dim']
    ...
    def forward(self, x, bool_masked_pos=None):
        ...
        if bool_masked_pos is None:
            bool_masked_pos = torch.zeros(batch_size, self.num_patches, dtype=torch.bool, device=x.device)
