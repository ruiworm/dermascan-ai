...
class Csv_TestDataset(Dataset):
    def __init__(self, input_filename, transforms, img_key, label_key, sep=",", tokenizer=None,parent_path=''):
        logging.debug(f'Loading csv data from {input_filename}.')
        ...
        self.transforms = transforms
        logging.debug('Done loading zero-shot evaluation data.')
...
class Csv_ValDataset(Dataset):
    def __init__(self, input_filename, transforms, img_key, label_key, sep="\t", tokenizer=None,parent_path=''):
        ...
        self.transforms = transforms
        logging.debug('Done loading val evaluation data.')
...
def get_csv_test_dataset(args, input_filename, preprocess_fn, is_train, epoch=0, tokenizer=None):
    assert input_filename
    dataset = Csv_TestDataset(
        input_filename,
        preprocess_fn,
        img_key=args.csv_img_key,
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None):
    ...
    if args.zeroshot_eval is not None:
        data["zeroshot_dermnet"] = get_csv_test_dataset(args, args.zeroshot_eval, preprocess_val, is_train=False, tokenizer=tokenizer)

    if args.zeroshot_eval1:
        data["zeroshot_pad"] = get_csv_test_dataset(args, args.zeroshot_eval1, preprocess_val, is_train=False, tokenizer=tokenizer)

    if args.zeroshot_eval2 is not None:
        data["zeroshot_ham"] = get_csv_test_dataset(args, args.zeroshot_eval2, preprocess_val, is_train=False, tokenizer=tokenizer)

    if args.zeroshot_eval3 is not None:
        data["zeroshot_SNU-134-classes"] = get_csv_test_dataset(args, args.zeroshot_eval3, preprocess_val, is_train=False, tokenizer=tokenizer)

    if args.zeroshot_eval4 is not None:
        data["zeroshot_SD-128-classes"] = get_csv_test_dataset(args, args.zeroshot_eval4, preprocess_val, is_train=False, tokenizer=tokenizer)

    if args.zeroshot_eval5 is not None:
        data["zeroshot_DAFFODIL-5-classes"] = get_csv_test_dataset(args, args.zeroshot_eval5, preprocess_val, is_train=False, tokenizer=tokenizer)

    if args.zeroshot_eval6 is not None:
        data["zeroshot_PH2_2-classes"] = get_csv_test_dataset(args, args.zeroshot_eval6, preprocess_val, is_train=False, tokenizer=tokenizer)
