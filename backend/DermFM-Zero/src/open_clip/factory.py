...
from .utils import call_PanDerm_large_visual, call_PanDerm_base_visual
...
def get_filtered_checkpoint_key(model, checkpoint_path):
    checkpoint_state_dict = get_checkpoint_weight(model, checkpoint_path)
...
def get_checkpoint_weight(model, checkpoint_path, strict=True):
    if Path(checkpoint_path).suffix in ('.npz', '.npy'):
        from .big_vision import load_big_vision_weights
        load_big_vision_weights(model, checkpoint_path)
        return {}
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if model_name == 'monet':
            ...
            try:
                ...
                raise RuntimeError("Please `pip install openai-clip` to use the 'monet' model.")
            ...
            model.eval()
            return model # Return early for monet
        ...
        linear_prob = model_kwargs.pop('linear_prob', False)
        finetune = model_kwargs.pop('finetune', False)
