...
def extract_features_from_dataloader(args, model, dataloader):
    ...
    for batch_idx, (batch, target, filename) in tqdm(
        ...
        with torch.inference_mode():
            ...
            """MONET, virtual_env: MILAN"""
            if args.model == 'MONET':
                embeddings = model.encode_image(batch)[:remaining, :].cpu()

            """dinov3"""
            if args.model == 'dinov3-b16' or args.model == 'dinov3-l16' or args.model == 'dinov3-7b':
                embeddings = model(batch)['pooler_output'][:remaining, :].cpu()

            """Model from open_clip API"""
