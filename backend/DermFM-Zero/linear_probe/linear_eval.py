...
def main(args):
    ...
    if 'hf-hub' in args.output_dir:
        args.output_dir = args.output_dir.replace(args.model, args.model.split('/')[-1])

    if args.checkpoint is not None and args.checkpoint.lower() != 'none':
        if args.model == 'PanDerm':
            state_dict = torch.load(args.checkpoint,map_location='cpu',
                        weights_only=True)
            state_dict = {k.replace("encoder.", ""): v for k, v in state_dict.items()}
            model.load_state_dict(state_dict, strict=False)
            print("Skipping checkpoint reload for PanDerm - weights already loaded correctly")
        else:
