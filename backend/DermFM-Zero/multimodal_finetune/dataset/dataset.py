...
from dataset.utils import strings2numeric
...
class Derm7PtDataset(object):
    ...
    # 血管结构：无\分叉\间隔\发卡形状\在回归\环状\点状\线性不规则
    ...
    def __init__(self, dir_images, metadata_df, train_indexes, valid_indexes, test_indexes, crop_amount=25):
        ...
        # Modify meta to include numeric labels in the columns. numeric数值的
        ...
        match_indexes = np.all(np.sort(np.concatenate((train_indexes, valid_indexes, test_indexes)))
                                   == range(len(self.df))) #np.alltrue测试数组的元素是否都为True类型
        ...
        self.train = self.df.iloc[train_indexes] #对行号进行索引
        self.valid = self.df.iloc[valid_indexes]
        self.test = self.df.iloc[test_indexes]

        # Meta data. (This apporach is only used for meta MLP encoder)
        ...
        # self.meta = pd.get_dummies(self.df[['elevation','sex','location','level_of_diagnostic_difficulty','management']])
        self.meta = pd.get_dummies(self.df[['elevation','sex','location']])
        self.meta_train = self.meta.iloc[train_indexes]
        self.meta_test = self.meta.iloc[test_indexes]
        self.meta_valid = self.meta.iloc[valid_indexes]

        self.meta_text_train = self.train[['elevation','sex','location']]
        self.meta_text_valid = self.valid[['elevation','sex','location']]
        self.meta_text_test = self.test[['elevation','sex','location']]


    def set_df_numeric_labels(self):
        """Add numeric values to the columns in the df.""" #向列中添加数值
    ...
    def get_tag_by_abbrev(self, abbrev):
        """Return the label info for a given abbreviation.""" #得到简称
    ...
    def labels2hot(self, labels, abbrev='DIAG'):
        ...
        one_hot_codes = np.eye(nb_classes)
        one_hot_labs = []
        for label in labels:
            one_hot_label = one_hot_codes[label]
            one_hot_labs.append(one_hot_label)
        return one_hot_labs
    ...
    def get_labels(self, data_type='all', one_hot=False):
        ...
        for abbrev in self.get_tag_abbrevs():
            labels = df[self.get_column_name_numeric(abbrev)].values.tolist()
            if one_hot:
