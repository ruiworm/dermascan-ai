...
class VITL14TextEncoder(nn.Module):
    def __init__(self, clip_model):
        super(VITL14TextEncoder, self).__init__()
    ...
    def text_global_pool(self, x, text: Optional[torch.Tensor] = None, pool_type: str = 'argmax'):
        if pool_type == 'first':
    ...
    def forward(self, text_tokens, normalize=False):

        return self.encode_text(text_tokens, normalize=normalize)

def create_vit_l14_text_encoder(pretrained='openai'):
    ...
    text_encoder = VITL14TextEncoder(clip_model)
