...
from sae.sparse_autoencoder.activation_resampler.abstract_activation_resampler import (
    AbstractActivationResampler,
    ParameterUpdateResults,
)
from sae.sparse_autoencoder.activation_resampler.utils.component_slice_tensor import (
    get_component_slice_tensor,
)
from sae.sparse_autoencoder.activation_store.base_store import ActivationStore
from sae.sparse_autoencoder.autoencoder.model import SparseAutoencoder
from sae.sparse_autoencoder.loss.abstract_loss import AbstractLoss
from sae.sparse_autoencoder.tensor_types import Axis
from sae.sparse_autoencoder.train.utils import get_model_device


class LossInputActivationsTuple(NamedTuple):
    """Loss and corresponding input activations tuple."""

    loss_per_item: Float[Tensor, Axis.names(
        Axis.BATCH, Axis.COMPONENT_OPTIONAL)]
...
class ActivationResampler(AbstractActivationResampler):
    ...
    _collated_neuron_activity: Float[Tensor, Axis.names(
    ...
    def __init__(
        ...
        threshold_is_dead_portion_fires: Annotated[float, Field(
            strict=True, ge=0, le=1)] = 0.0, 
        resample_epoch_freq= None,
    ) -> None:
        ...
        self.neuron_activity_window_start = resample_interval - \
        ...
        self.resample_epoch_freq = resample_epoch_freq
        self.epoch_since_last_resample = 0
    ...
    def _get_dead_neuron_indices(
        ...
        threshold_is_dead_n_fires: int = int(
            self._n_activations_collated_since_last_resample *
            self._threshold_is_dead_portion_fires
        )
    ...
    def compute_loss_and_get_activations(
        ...
        with torch.no_grad():
            ...
            input_activations_batches: list[
                Float[Tensor, Axis.names(
                    Axis.BATCH, Axis.INPUT_OUTPUT_FEATURE)]
            ] = []
            # shuffle=True, as the model is trained on the same dataset 
            # for multiple epochs (in original SAE work in language domain, 
            # the model did not see the data twice, so 'shuffle' did not matter)
            dataloader = DataLoader(
                store, batch_size=train_batch_size, shuffle=True)
            ...
            input_activations = torch.cat(
                input_activations_batches).to(model_device)
    ...
    def sample_input(
        ...
        input_activations: Float[
            Tensor, Axis.names(
                Axis.BATCH, Axis.COMPONENT_OPTIONAL, Axis.INPUT_OUTPUT_FEATURE)
        ],
        n_samples: list[int],
        ...
        sampled_inputs: list[
            Float[Tensor, Axis.names(
                Axis.DEAD_FEATURE, Axis.INPUT_OUTPUT_FEATURE)]
        ] = []
        ...
        for component_idx, component_n_samples in enumerate(n_samples):
            ...
            sampled_inputs.append(
                component_input_activations[component_sample_indices, :])
    ...
    def renormalize_and_scale(
        ...
        """
        alive_neuron_mask: Bool[Tensor,
                                " learned_features"] = neuron_activity > 0
    ...
    def resample_dead_neurons(
        ...
        with torch.no_grad():
            ...
            tmp_dead_neuron_indices = copy.deepcopy(dead_neuron_indices)
            dead_neuron_indices =[]
            for dead_neuron_index in tmp_dead_neuron_indices:
                # dead_neuron_indices.append(dead_neuron_index.to(dead_decoder_weight_updates.device))
                dead_neuron_indices.append(dead_neuron_index.to('cuda'))
            ...
            for component_idx in range(self._n_components):
                ...
                renormalized_input: Float[
                    Tensor, Axis.names(Axis.DEAD_FEATURE,
                                       Axis.INPUT_OUTPUT_FEATURE)
                ...
                encoder_weight: Float[
                    Tensor, Axis.names(Axis.LEARNT_FEATURE,
                                       Axis.INPUT_OUTPUT_FEATURE)
    ...
    def step_resampler(
        ...
        self.epoch_since_last_resample += 1
        if self._n_times_resampled < self._max_n_resamples:
            ...
            if (self._activations_seen_since_last_resample >= self.neuron_activity_window_end) and (self.epoch_since_last_resample >=self.resample_epoch_freq):
                print(f"Resampling now....")
